...
const AudioAnalyzer: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            console.error(e);
            setError("Failed to analyze audio. Please try a shorter clip or different format.");
        } finally {
    ...
    return (
        <Page title="AI Audio Critic" description="Get professional feedback, technical analysis, and genre classification for your tracks.">
                ...
                <div className="md:col-span-2">
                    {analysis ? (
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <Card className="text-center py-4 bg-indigo-900/20 border-indigo-500/30">
                                    <p className="text-xs text-indigo-300 uppercase font-bold">BPM</p>
                                ...
                                <Card className="text-center py-4 bg-purple-900/20 border-purple-500/30">
                                    <p className="text-xs text-purple-300 uppercase font-bold">Key</p>
                                    <p className="text-2xl font-bold text-white">{analysis.key}</p>
                                </Card>
                                <Card className="text-center py-4 bg-pink-900/20 border-pink-500/30">
                                    <p className="text-xs text-pink-300 uppercase font-bold">Genre</p>
                                    <p className="text-lg font-bold text-white">{analysis.genre}</p>
                                </Card>
                                <Card className="text-center py-4 bg-blue-900/20 border-blue-500/30">
                                    <p className="text-xs text-blue-300 uppercase font-bold">Mood</p>
                                    <p className="text-lg font-bold text-white">{analysis.mood}</p>
                                </Card>
                            </div>
                            
                            <Card>
                                <h3 className="text-xl font-semibold mb-4 text-indigo-300">Production Feedback</h3>
                                <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">{analysis.productionFeedback}</p>
                            </Card>
                            ...
                            <Card>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {analysis.chords.map((chord, i) => (
                                        <span key={i} className="px-4 py-2 bg-gray-700 rounded-lg font-mono font-bold text-indigo-200 border border-gray-600">
                                            {chord}
                                        </span>
                                    ))}
                                </div>
                        ...
                        <div className="h-full flex items-center justify-center bg-gray-800/30 rounded-lg border-2 border-dashed border-gray-700 min-h-[400px]">
                            <p className="text-gray-500">Upload a track and click Analyze to see AI insights.</p>
                        </div>
                    )}
                </div>
            </div>
