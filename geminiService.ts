        ...
        properties: {
            sections: {
                ...
                items: {
                    type: Type.OBJECT,
                    properties: {
                        name: { type: Type.STRING, description: "Section name (e.g. Intro, Verse 1, Chorus)" },
                        description: { type: Type.STRING, description: "Brief production notes and mood for this section" },
                        bars: { type: Type.INTEGER, description: "Estimated number of bars" }
                    },
...
const _internalGenerateAudio = async (detailedPrompt: string): Promise<string> => {
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
            ...
            systemInstruction: "You are a professional musical synthesizer and vocal instrument. You do NOT speak words. You ONLY produce rhythmic, melodic, and harmonic sounds. Perform the requested musical arrangement using advanced vocal percussion, bass humming, and tonal synthesis.",
            speechConfig: {
                voiceConfig: {
...
export const processAudioTrack = async (audioBase64: string, prompt: string): Promise<string> => {
    const ai = getAi();
...
export const analyzeRhymeAndMeter = async (line: string, context: string): Promise<string> => {
    const ai = getAi();
...
export const generateLineAlternatives = async (line: string, type: string): Promise<string[]> => {
    const ai = getAi();
    const responseSchema = {
        type: Type.OBJECT,
        properties: {
...
export const analyzeAudioTrack = async (audioBase64: string, mimeType: string): Promise<AudioAnalysisResult> => {
    const ai = getAi();
    const responseSchema = {
        type: Type.OBJECT,
        properties: {
...
export const remixAudioTrack = async (audioBase64: string, remixInstruction: string): Promise<string> => {
    return processAudioTrack(audioBase64, `Creative Remix Task: ${remixInstruction}. Transform the input audio while keeping the core rhythm or melody as appropriate.`);
};
...
export const generateMarketingPackage = async (lyrics: string, concept: string, targetAudience: string, previousPackage?: SocialMarketingPackage | null): Promise<SocialMarketingPackage> => {
    const ai = getAi();
    const responseSchema = {
        type: Type.OBJECT,
        properties: {
