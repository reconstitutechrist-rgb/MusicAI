...
/**
 * Main export function - renders mix and triggers download (legacy WAV)
 */
...
/**
 * Export mix with format selection
 */
...
/**
 * Export individual stems (legacy WAV)
 */
...
/**
 * Export individual stems with format selection
 */
export async function exportStems(
  ...
  for (let i = 0; i < stems.length; i++) {
    ...
    const renderedBuffer = await renderStemOffline(track, applyFX, sampleRate);
    const blob = await bufferToBlob(renderedBuffer, format);
    downloadBlob(blob, `${name}.${config.extension}`);
