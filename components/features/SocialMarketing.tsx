...
const SocialMarketing: React.FC<SocialMarketingProps> = ({ lyrics, songConcept }) => {
    ...
    // New State for Image Customization
    const [selectedMedium, setSelectedMedium] = useState('None');
    const [selectedPalette, setSelectedPalette] = useState('None');
    const [selectedLighting, setSelectedLighting] = useState('None');

    const MEDIUM_OPTIONS = ['None', 'Cinematic Photography', 'Digital Art', 'Oil Painting', 'Watercolor', '3D Render', 'Anime', 'Vintage Polaroid', 'Cyberpunk', 'Minimalist Line Art'];
    const PALETTE_OPTIONS = ['None', 'Vibrant Neon', 'Pastel Dream', 'Dark & Moody', 'Black & White', 'Warm Earth Tones', 'Cool Blues', 'Retro Vaporwave', 'Monochrome'];
    const LIGHTING_OPTIONS = ['None', 'Golden Hour', 'Studio Lighting', 'Neon Glow', 'Natural Sunlight', 'Dramatic Shadows', 'Cinematic Haze', 'Soft Diffused'];

    // Video State
    const [apiKeySelected, setApiKeySelected] = useState(false);
    const [videoPrompts, setVideoPrompts] = useState<string[]>([]);
    // Changed to support multiple concurrent generations
    const [generatingVideoIndices, setGeneratingVideoIndices] = useState<number[]>([]);
    ...
    const generateImageInternal = async (prompt: string) => {
    ...
    const generateVideoInternal = async (prompt: string, index: number) => {
        if (!apiKeySelected) {
            const hasKey = await (window as any).aistudio.hasSelectedApiKey();
            if(!hasKey) {
        ...
        setGeneratingVideoIndices(prev => [...prev, index]);
        ...
        try {
            ...
            setGeneratingVideoIndices(prev => prev.filter(i => i !== index));
        }
    };
    ...
    const handleGenerate = async () => {
        ...
        // Capture previous result to prompt for something new
        ...
        setVideoErrors({});
        try {
            const pkg = await generateMarketingPackage(lyrics, songConcept, targetAudience, previousResult);
            setResult(pkg);
    ...
    const handleGenerateFullCampaign = async () => {
        ...
        // Ensure API key is ready for videos
        if (!apiKeySelected) {
             const hasKey = await (window as any).aistudio.hasSelectedApiKey();
             if(!hasKey) {
                 await (window as any).aistudio.openSelectKey();
        ...
        try {
            // 1. Generate Text Package, passing previous result to ensure freshness
            const pkg = await generateMarketingPackage(lyrics, songConcept, targetAudience, previousResult);
            setResult(pkg);
            setImagePrompt(pkg.imagePrompt || '');
            setVideoPrompts(pkg.videoPrompts || []);
            ...
            if (pkg.imagePrompt) {
                generateImageInternal(pkg.imagePrompt); // Fire and forget, handled by internal state
            }

            // 3. Auto-generate Videos
            if (pkg.videoPrompts && pkg.videoPrompts.length > 0) {
                 pkg.videoPrompts.forEach((prompt, index) => {
                     generateVideoInternal(prompt, index); // Fire and forget, handled by internal state
                 });
            }

        } catch (e) {
            setError('Failed to generate full campaign.');
            console.error(e);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleGenerateImage = () => {
        ...
        // Construct prompt with modifiers
        const modifiers = [];
        if (selectedMedium && selectedMedium !== 'None') modifiers.push(`${selectedMedium} style`);
        if (selectedPalette && selectedPalette !== 'None') modifiers.push(`${selectedPalette} color palette`);
        if (selectedLighting && selectedLighting !== 'None') modifiers.push(`${selectedLighting} lighting`);
        
        if (modifiers.length > 0) {
            finalPrompt = `${modifiers.join(', ')}. ${finalPrompt}`;
        }

        generateImageInternal(finalPrompt);
    };

    const handleGenerateVideo = (prompt: string, index: number) => {
        generateVideoInternal(prompt, index);
    };
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-4">
                         <p className="text-gray-400 mt-1">
                            {lyrics ? "Create a campaign below, or load a saved one." : "First, create a song in the 'Create' tab."}
                         </p>
                         <div className="flex items-center gap-4">
                            {result && <Button onClick={handleSaveCampaign} variant="secondary">Save Campaign</Button>}
                            
                            <Button onClick={handleGenerate} variant="secondary" isLoading={isLoading} disabled={!lyrics || !songConcept}>
                                {result ? 'Regenerate Text' : 'Generate Text Only'}
                            </Button>
                            
                            <Button onClick={handleGenerateFullCampaign} variant="primary" isLoading={isLoading} disabled={!lyrics || !songConcept}>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="col-span-2">
                                        <label htmlFor="aspect-ratio-select" className="block text-xs font-medium text-gray-400 mb-1">Aspect Ratio</label>
                                        <select 
                                    ...
                                    <div className="col-span-2">
                                        <h4 className="text-xs font-semibold text-gray-300 uppercase tracking-wider mb-2 mt-2">Style Modifiers</h4>
                                    </div>
                                    <div>
                                        <label htmlFor="medium-select" className="block text-xs font-medium text-gray-400 mb-1">Medium</label>
                                        <select 
                                            id="medium-select"
                                    ...
                                    <div>
                                        <label htmlFor="palette-select" className="block text-xs font-medium text-gray-400 mb-1">Palette</label>
                                        <select 
                                            id="palette-select"
                                            ...
                                            className="block w-full bg-gray-900 border-gray-600 rounded-md text-xs py-2"
                                        >
                                            {PALETTE_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label htmlFor="lighting-select" className="block text-xs font-medium text-gray-400 mb-1">Lighting</label>
                                        <select 
                                            id="lighting-select"
                                ...
                                <Button onClick={handleGenerateImage} isLoading={isGeneratingImage} disabled={!imagePrompt} className="w-full mt-2">
                        ...
                        <Card>
                            ...
                            <div className="space-y-6">
                                {result.captions.map(p => {
                                    ...
                                    const rawIndex = activeCaptionIndexes[platform] || 0;
                                    const currentIndex = (p.variations && p.variations.length > 0) ? rawIndex % p.variations.length : 0;
                                    const currentCaption = p.variations?.[currentIndex] || "No caption available.";
                                            ...
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                <div className="flex items-center gap-1">
                                                     <span className="text-xs text-gray-400">
                                                        Variation {currentIndex + 1} of {p.variations.length}
                                                     </span>
                                                    <Button 
                                                        size="sm" 
                                                        variant="ghost" 
                                                        className="px-1 py-1 h-6 w-6 disabled:opacity-30 disabled:cursor-not-allowed" 
                                                        onClick={() => handleCaptionCycle(platform, 'prev', p.variations.length)}
                                                        disabled={p.variations.length <= 1}
                                                    >&lt;</Button>
                                                    <Button 
                                                        size="sm" 
                                                        variant="ghost" 
                                                        className="px-1 py-1 h-6 w-6 disabled:opacity-30 disabled:cursor-not-allowed" 
                                                        onClick={() => handleCaptionCycle(platform, 'next', p.variations.length)}
                                                        disabled={p.variations.length <= 1}
                                                    >&gt;</Button>
                                                </div>
                                            </div>
                            ...
                            <div className="space-y-4 mt-4">
                                {videoPrompts.map((prompt, i) => (
                                     <div key={i} className="p-3 bg-gray-900/50 rounded-md">
                                        ...
                                        <Button 
                                            onClick={() => handleGenerateVideo(prompt, i)} 
                                            isLoading={generatingVideoIndices.includes(i)} 
                                            disabled={generatingVideoIndices.includes(i)} 
                                            className="w-full text-sm"
                                        >
                                            {videoUrls[i] ? `Regenerate Video ${i + 1}` : `Generate Video ${i + 1}`}
                                        </Button>
                                        {generatingVideoIndices.includes(i) && (
