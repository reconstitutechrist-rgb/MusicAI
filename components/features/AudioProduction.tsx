...
import MultibandCompressor, {
  DEFAULT_MULTIBAND_SETTINGS,
  type MultibandCompressorSettings,
} from "../ui/MultibandCompressor";
import MasteringAssistant, {
  type MasteringSuggestions,
} from "../ui/MasteringAssistant";
...
const AudioProduction: React.FC<AudioProductionProps> = ({
  lyrics,
  instrumentalUrl,
  initialVocalUrl,
  karaokeSongs = [],
  ...
  const [currentInstrumentalUrl, setCurrentInstrumentalUrl] = useState<
    string | undefined
  >(instrumentalUrl);
  ...
  const [harmonyEQBands, setHarmonyEQBands] =
    useState<EQBand[]>(DEFAULT_EQ_BANDS);
  ...
  const [harmonyAnalyser, setHarmonyAnalyser] = useState<AnalyserNode | null>(
    null,
  );
  ...
  const [spectrumMode, setSpectrumMode] = useState<"bars" | "line" | "filled">(
    "bars",
  );
  const [lufsPreset, setLufsPreset] = useState<LufsPreset>("spotify");
  const [multibandSettings, setMultibandSettings] =
    useState<MultibandCompressorSettings>(DEFAULT_MULTIBAND_SETTINGS);
  const [multibandBypass, setMultibandBypass] = useState(true);
  const [multibandGainReductions, setMultibandGainReductions] = useState<
    number[]
  >([0, 0, 0, 0]);

  // Tempo-synced delay state
  const [delayBpm, setDelayBpm] = useState(120);
  const [delayNoteValue, setDelayNoteValue] = useState<
    "1/4" | "1/8" | "1/8d" | "1/16"
  >("1/8");
  ...
  const [sidechainSource, setSidechainSource] = useState<
    "inst" | "vocal" | "harmony"
  >("inst");
  const [sidechainTarget, setSidechainTarget] = useState<"vocal" | "harmony">(
    "vocal",
  );
  const [sidechainGainReduction, setSidechainGainReduction] = useState(0);
  const [showSidechain, setShowSidechain] = useState(false);
  const [sidechainSettings, setSidechainSettings] = useState<SidechainSettings>(
    DEFAULT_SIDECHAIN_SETTINGS,
  );
  ...
  const [masterAnalyserL, setMasterAnalyserL] = useState<AnalyserNode | null>(
    null,
  );
  const [masterAnalyserR, setMasterAnalyserR] = useState<AnalyserNode | null>(
    null,
  );
  ...
  const [showInstrumentalSwapModal, setShowInstrumentalSwapModal] =
    useState(false);
  const [isGeneratingInstrumental, setIsGeneratingInstrumental] =
    useState(false);
  ...
  const [referenceTrackUrl, setReferenceTrackUrl] = useState<string | null>(
    null,
  );
  ...
  const [referenceAnalyser, setReferenceAnalyser] =
  ...
  const handleReferenceTrackUpload = useCallback(
  ...
  const setupReferenceAudioGraph = useCallback(
  ...
  const setupTrackGraph = useCallback(
    (
      ...
      if (!refs.gain) {
        ...
        const outputNode =
          graphRefs.current.master?.masterGain || ctx.destination;
  ...
  const setupInstrumentalGraph = useCallback(
    (element: HTMLAudioElement, volume: number) => {
  ...
  useEffect(() => {
    ...
    if (vocalDelay) {
      vocalDelay.delayTime.setTargetAtTime(
        delayTime,
    ...
    if (harmonyDelay) {
      harmonyDelay.delayTime.setTargetAtTime(
        delayTime,
        audioContext.currentTime,
        rampTime,
      );
    ...
    if (vocalFeedback) {
      vocalFeedback.gain.setTargetAtTime(
        delayFeedback,
        audioContext.currentTime,
        rampTime,
      );
    ...
    if (harmonyFeedback) {
      harmonyFeedback.gain.setTargetAtTime(
        delayFeedback,
        audioContext.currentTime,
        rampTime,
      );
  ...
  const setupMasterBus = useCallback(() => {
    ...
    if (!master.masterGain) {
      ...
      master.multibandCompressor = createMultibandCompressor(
        ctx,
        multibandSettings,
      );
  ...
  useEffect(() => {
    const updateGainReductions = () => {
      ...
      gainReductionAnimationRef.current =
        requestAnimationFrame(updateGainReductions);
    };
  ...
  useEffect(() => {
    ...
    const sourceTrack =
      sidechainSource === "inst" ? "instrumental" : sidechainSource;
    ...
    if (!sidechainNodesRef.current) {
      sidechainNodesRef.current = createSidechainCompressor(
        audioContext,
        sidechainSettings,
      );
    }
    ...
    const processSidechain = () => {
      ...
      const attackCoeff =
        1 - Math.exp(-1 / (audioContext.sampleRate * sidechainSettings.attack));
      const releaseCoeff =
        1 -
        Math.exp(-1 / (audioContext.sampleRate * sidechainSettings.release));

      // Use attack for decreasing gain, release for increasing
      const coeff =
        gainMultiplier < targetGainValue ? attackCoeff : releaseCoeff;
      const newGain =
        targetGainValue + (gainMultiplier - targetGainValue) * coeff;
      ...
      if (targetRefs?.gain) {
        ...
        const trackVolume =
          targetTrack === "vocal" ? vocalVolume : harmonyVolume;
        const adjustedVolume = trackVolume * newGain;
        targetRefs.gain.gain.setTargetAtTime(
          adjustedVolume,
          currentTime,
          rampTime,
        );
  ...
  const getEffectiveEQSettings = useCallback(
    (settings: TrackFX, eqBands: EQBand[], useAdvanced: boolean): TrackFX => {
      ...
      const midBands = enabledBands.filter(
        (b) => b.frequency >= 500 && b.frequency < 2000,
      );
      const highBands = enabledBands.filter((b) => b.frequency >= 2000);

      const avgGain = (bands: EQBand[]) =>
        bands.length > 0
          ? bands.reduce((sum, b) => sum + b.gain, 0) / bands.length
          : 0;
  ...
  useEffect(() => {
    if (vocalRef.current) {
      ...
      effectiveSettings = getEffectiveEQSettings(
        effectiveSettings,
        vocalEQBands,
        useAdvancedEQ && !bypassAllFX,
      );
      setupTrackGraph(
        "vocal",
        vocalRef.current,
  ...
  useEffect(() => {
    ...
    const updateParametricEQ = (
      ...
      if (useAdvanced) {
        ...
        if (trackRefs.eqLow)
          trackRefs.eqLow.gain.setTargetAtTime(
            0,
            audioContext.currentTime,
            rampTime,
          );
        if (trackRefs.eqMid)
          trackRefs.eqMid.gain.setTargetAtTime(
            0,
            audioContext.currentTime,
            rampTime,
          );
        if (trackRefs.eqHigh)
          trackRefs.eqHigh.gain.setTargetAtTime(
            0,
            audioContext.currentTime,
            rampTime,
          );
  ...
  const handleGenerateNewInstrumental = useCallback(async () => {
    ...
    try {
      ...
      setInstrumentalError(
        "Failed to generate instrumental. Please try again.",
      );
      console.error(e);
    } finally {
  ...
  const handleApplyMasteringSuggestions = (
    suggestions: MasteringSuggestions,
  ) => {
    ...
    const presetMap: Record<string, MultibandCompressorSettings> = {
      ...
      punch: {
        ...DEFAULT_MULTIBAND_SETTINGS,
        bands: DEFAULT_MULTIBAND_SETTINGS.bands.map((b) => ({
          ...b,
          threshold: b.threshold - 4,
          ratio: b.ratio + 1,
        })),
      },
      broadcast: {
        ...
        bands: DEFAULT_MULTIBAND_SETTINGS.bands.map((b) => ({
          ...b,
          threshold: b.threshold - 6,
          ratio: b.ratio + 2,
        })),
      },
      vocal: {
        ...DEFAULT_MULTIBAND_SETTINGS,
        bands: DEFAULT_MULTIBAND_SETTINGS.bands.map((b, i) =>
          i === 2
            ? { ...b, threshold: b.threshold - 3, makeupGain: b.makeupGain + 2 }
            : b,
        ),
      },
    };
  ...
  useEffect(() => {
    ...
    const vocalUrl =
      useRefinedVocal && refinedVocalUrl ? refinedVocalUrl : vocalAudioUrl;
  ...
  const handleExportMix = async () => {
    ...
    try {
      ...
      const renderedBuffer = await renderMixOffline(
        tracks,
        44100,
        (progress) => {
  ...
  const handleExportStems = async () => {
    ...
    try {
      ...
      if (exportOptions.includeVocal && vocalBufferRef.current) {
        const effectiveVocalFX = getEffectiveEQSettings(
          vocalSettings,
          vocalEQBands,
          useAdvancedEQ,
        );
      ...
      if (exportOptions.includeHarmony && harmonyBufferRef.current) {
        const effectiveHarmonyFX = getEffectiveEQSettings(
          harmonySettings,
          harmonyEQBands,
          useAdvancedEQ,
        );
      ...
      for (let i = 0; i < stems.length; i++) {
        ...
        const renderedBuffer = await renderStemOffline(
          trackConfig,
          exportOptions.applyFXToStems,
          44100,
        );
        const blob = await bufferToBlob(renderedBuffer, exportFormat);
    ...
    reader.onload = (e) => {
      try {
        ...
        const importedPresets: FXPreset[] = data.presets
          .filter(
            (p: FXPreset) =>
              p.id && p.name && p.vocalFX && p.harmonyFX && p.volumes,
          )
          .map((p: FXPreset) => ({
            ...p,
        ...
        alert(
          "Failed to parse preset file. Make sure it is a valid JSON file.",
        );
      }
    };
    ...
    if (currentPresetId) {
      ...
      if (preset) {
        const settingsMatch =
          ...
          JSON.stringify(harmonySettings) ===
            JSON.stringify(preset.harmonyFX) &&
  ...
  const updateAutomationLane = useCallback(
    (updatedLane: AutomationLaneData) => {
  ...
  useEffect(() => {
    if (!automationEnabled || !isPlayingMix || automationLanes.length === 0)
      return;
  ...
  const FXControls = ({
    ...
    <div className="bg-gray-800 p-4 rounded-md mt-2 space-y-4 border border-gray-700 animate-in fade-in slide-in-from-top-2 duration-200">
      ...
      {useAdvancedEQ ? (
        <ParametricEQ bands={eqBands} onChange={setEQBands} height={140} />
      ) : (
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({ ...settings, eqLow: Number(e.target.value) })
              }
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({ ...settings, eqMid: Number(e.target.value) })
              }
              className="w-full h-1 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-indigo-500"
            />
            <div className="text-center text-xs text-gray-500">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({ ...settings, eqHigh: Number(e.target.value) })
              }
              className="w-full h-1 bg-gray-600 rounded-lg appearance-none cursor-pointer accent-indigo-500"
            />
            <div className="text-center text-xs text-gray-500">
      ...
      <div className="grid grid-cols-2 gap-4 border-t border-gray-700 pt-3">
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setSettings({ ...settings, reverb: Number(e.target.value) })
            }
  ...
  return (
    <Page
      title="Audio Production"
      description="Bring your lyrics to life by generating vocal tracks and polishing your final mix with AI mastering."
    >
          ...
          <Card>
            ...
            {isEditingLyrics ? (
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 text-center">
                    Instrumental will be preserved. Only vocals will be
                    regenerated.
                  </p>
                </div>
              </div>
              ...
              <p className="text-gray-400 whitespace-pre-wrap h-96 overflow-y-auto">
                {editedLyrics ||
                  "No lyrics generated yet. Go to the 'Create' tab first."}
              </p>
            )}
          </Card>
        </div>
        <div className="md:col-span-2 space-y-8">
          ...
          {vocalAudioUrl ? (
            <Card className="border-indigo-500/50">
              ...
              {!currentInstrumentalUrl && (
                <div className="bg-gray-900/50 p-4 rounded-lg border border-dashed border-gray-600 mb-4 text-center">
                  ...
                  <input
                    ...
                    className="text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:bg-indigo-600 file:text-white file:border-none hover:file:bg-indigo-700 cursor-pointer"
                  />
                </div>
              )}
              ...
              <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {currentInstrumentalUrl ? (
                      <input
                        ...
                        value={instVolume}
                        onChange={(e) =>
                          setInstVolume(parseFloat(e.target.value))
                        }
                ...
                <div className="bg-gray-900/50 p-3 rounded-lg">
                  <div className="flex items-center gap-4">
                    ...
                    <button
                      onClick={() => setShowVocalFX(!showVocalFX)}
                      className={`text-xs px-2 py-1 rounded border ${showVocalFX ? "bg-indigo-600 border-indigo-500 text-white" : "border-gray-600 text-gray-400"}`}
                    >
                      FX
                    </button>
                  </div>
                  ...
                  {refinedVocalUrl && (
                    <div className="flex items-center gap-2 ml-28 mt-2">
                      <label className="inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={useRefinedVocal}
                          onChange={(e) => setUseRefinedVocal(e.target.checked)}
                        ...
                        <span className="ms-3 text-xs font-medium text-gray-300">
                          {useRefinedVocal ? "Using Refined" : "Using Original"}
                        </span>
                      </label>
                    </div>
                  )}
                  ...
                  {showVocalFX && (
                    <FXControls
                      ...
                      setSettings={setVocalSettings}
                      eqBands={vocalEQBands}
                      setEQBands={setVocalEQBands}
                      trackName="Vocal"
                    />
                  )}
                </div>
                    ...
                    {harmonyAudioUrl ? (
                      <>
                        <input
                          ...
                          value={harmonyVolume}
                          onChange={(e) =>
                            setHarmonyVolume(parseFloat(e.target.value))
                          }
                        ...
                        <button
                          onClick={() => setShowHarmonyFX(!showHarmonyFX)}
                          className={`text-xs px-2 py-1 rounded border ${showHarmonyFX ? "bg-indigo-600 border-indigo-500 text-white" : "border-gray-600 text-gray-400"}`}
                        >
                          FX
                        </button>
                      </>
                      ...
                      <div className="flex-1 text-xs text-gray-500 italic">
                        No harmony generated
                      </div>
                    )}
                  </div>
                  {harmonyAudioUrl && showHarmonyFX && (
                    <FXControls
                      settings={harmonySettings}
                      setSettings={setHarmonySettings}
                      eqBands={harmonyEQBands}
                      setEQBands={setHarmonyEQBands}
                      trackName="Harmony"
                    />
                  )}
                </div>
                ...
                {currentInstrumentalUrl && (
                  <audio
                    ...
                    preload="auto"
                    onEnded={() => setIsPlayingMix(false)}
                    onPlay={() => {
                      if (audioContext?.state === "suspended")
                        audioContext.resume();
                    }}
                    onLoadedMetadata={(e) =>
                      setupInstrumentalGraph(e.currentTarget, instVolume)
                    }
                  />
                )}
                <audio
                  ref={vocalRef}
                  src={
                    useRefinedVocal && refinedVocalUrl
                      ? refinedVocalUrl
                      : vocalAudioUrl
                  }
                  crossOrigin="anonymous"
                ...
                <div className="flex gap-2 mt-2">
                  <Button
                    ...
                    className="flex-1"
                    disabled={!currentInstrumentalUrl && !vocalAudioUrl}
                  >
                ...
                <div className="mt-3 pt-3 border-t border-gray-700">
                  ...
                  >
                    ...
                    {bypassAllFX
                      ? "A (Original - FX Bypassed)"
                      : "B (Processed - FX Active)"}
                    <kbd className="ml-2 px-1.5 py-0.5 text-xs bg-gray-700 rounded">
                      B
                    </kbd>
                  </button>
                  <p className="text-xs text-gray-500 mt-1 text-center">
                    Compare original vs. processed sound
                  </p>
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        onChange={(e) => {
                          if (e.target.value) {
                            addAutomationLane(
                              e.target.value as AutomatableParameter,
                            );
                            e.target.value = "";
                          }
                        }}
                      ...
                      >
                        ...
                        {AUTOMATABLE_PARAMS.filter(
                          (p) =>
                            !automationLanes.some((l) => l.parameter === p.id),
                        ).map((param) => (
                      ...
                      {automationLanes.length === 0 ? (
                        <div className="text-center py-6 text-gray-500 text-sm">
                          ...
                          <p className="text-xs mt-1">
                            Select a parameter above to add an automation lane.
                          </p>
                        </div>
                      ) : (
                        automationLanes.map((lane) => {
                          const paramConfig = AUTOMATABLE_PARAMS.find(
                            (p) => p.id === lane.parameter,
                          );
                          return (
                            <AutomationLane
                              key={lane.id}
                              ...
                              parameterLabel={
                                paramConfig?.label || lane.parameter
                              }
                  ...
                  <div className="mt-3 p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs font-mono text-orange-400">
                          {sidechainGainReduction.toFixed(1)} dB
                        </span>
                      </div>
                      <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, Math.abs(sidechainGainReduction) * 5)}%`,
                          }}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-gray-300 font-mono">
                          {sidechainSettings.threshold.toFixed(1)} dB
                        </span>
                      </div>
                      <input
                        type="range"
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-gray-300 font-mono">
                          {sidechainSettings.ratio.toFixed(1)}:1
                        </span>
                      </div>
                      <input
                        type="range"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className="text-gray-300 font-mono">
                            {(sidechainSettings.attack * 1000).toFixed(0)}ms
                          </span>
                        </div>
                        <input
                          type="range"
                      ...
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className="text-gray-300 font-mono">
                            {(sidechainSettings.release * 1000).toFixed(0)}ms
                          </span>
                        </div>
                        <input
                          type="range"
                    ...
                    <p className="text-xs text-gray-500">
                      Sidechain compression ducks the target track when the
                      source track plays. Common use: duck vocals/harmonies when
                      the instrumental beat hits.
                    </p>
                  </div>
                )}
                  ...
                  <div className="mt-3 p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-4">
                        ...
                        {!referenceTrackUrl ? (
                          ...
                          <div className="space-y-3">
                            ...
                            <audio
                              ...
                              onLoadedMetadata={(e) =>
                                setupReferenceAudioGraph(e.currentTarget)
                              }
                              onEnded={() => {
                                setReferenceTrackPlaying(false);
                                setListeningToReference(false);
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <select
                            ...
                            onChange={(e) =>
                              setSpectrumMode(
                                e.target.value as "bars" | "line" | "filled",
                              )
                            }
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setLimiterCeiling(parseFloat(e.target.value))
                            }
                        ...
                        <div className="flex items-center justify-center">
                          ...
                          <span
                            className={`text-lg font-mono ${limiterGainReduction < -0.5 ? "text-orange-400" : "text-green-400"}`}
                          >
                    ...
                    <div className="mt-3 p-3 bg-gray-800 rounded-lg">
                      ...
                      <div className="grid grid-cols-4 gap-2 mt-2">
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            value={delayBpm}
                            onChange={(e) =>
                              setDelayBpm(parseInt(e.target.value))
                            }
                            className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                          />
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            value={delayMix}
                            onChange={(e) =>
                              setDelayMix(parseFloat(e.target.value))
                            }
                            className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                          />
                        </div>
                      ...
                      <div className="text-center mt-2 text-xs text-gray-500">
                        Delay Time:{" "}
                        {(
                          calculateDelayTime(delayBpm, delayNoteValue) * 1000
                        ).toFixed(0)}
                        ms
                      </div>
                    </div>
                ...
                {showAITools && (
                  <div className="mt-3 p-4 bg-gray-900/50 rounded-lg border border-gray-700 space-y-4">
                    ...
                    {/* Analyze Button */}
                    {!audioAnalysis &&
                      (showMasteringAssistant || showChordSuggestions) && (
                        <button
                          onClick={handleAnalyzeAudio}
                          disabled={
                            isAnalyzingAudio ||
                            (!currentInstrumentalUrl && !vocalAudioUrl)
                          }
                  ...
                  >
                      ...
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setExportOptions({
                              ...exportOptions,
                              includeInstrumental: e.target.checked,
                            })
                          }
                        ...
                        />
                        <span
                          className={`text-sm ${currentInstrumentalUrl ? "text-gray-200" : "text-gray-500"}`}
                        >
                          Instrumental{" "}
                          {!currentInstrumentalUrl && "(not loaded)"}
                        </span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                        ...
                        />
                        <span
                          className={`text-sm ${vocalAudioUrl ? "text-gray-200" : "text-gray-500"}`}
                        >
                          Vocal {useRefinedVocal ? "(Refined)" : "(Original)"}{" "}
                          {!vocalAudioUrl && "(not generated)"}
                        </span>
                      </label>
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                        ...
                        />
                        <span
                          className={`text-sm ${harmonyAudioUrl ? "text-gray-200" : "text-gray-500"}`}
                        >
                    ...
                    <div className="mb-4">
                      ...
                      <select
                        value={exportFormat}
                        onChange={(e) =>
                          setExportFormat(e.target.value as AudioFormat)
                        }
                      ...
                      >
                        <optgroup label="WAV (Lossless)">
                          <option value="wav-16">
                            {AUDIO_FORMATS["wav-16"].name} -{" "}
                            {AUDIO_FORMATS["wav-16"].description}
                          </option>
                          <option value="wav-24">
                            {AUDIO_FORMATS["wav-24"].name} -{" "}
                            {AUDIO_FORMATS["wav-24"].description}
                          </option>
                          <option value="wav-32">
                            {AUDIO_FORMATS["wav-32"].name} -{" "}
                            {AUDIO_FORMATS["wav-32"].description}
                          </option>
                        </optgroup>
                        <optgroup label="Compressed">
                          <option value="mp3-128">
                            {AUDIO_FORMATS["mp3-128"].name} -{" "}
                            {AUDIO_FORMATS["mp3-128"].description}
                          </option>
                          <option value="mp3-192">
                            {AUDIO_FORMATS["mp3-192"].name} -{" "}
                            {AUDIO_FORMATS["mp3-192"].description}
                          </option>
                          <option value="mp3-256">
                            {AUDIO_FORMATS["mp3-256"].name} -{" "}
                            {AUDIO_FORMATS["mp3-256"].description}
                          </option>
                          <option value="mp3-320">
                            {AUDIO_FORMATS["mp3-320"].name} -{" "}
                            {AUDIO_FORMATS["mp3-320"].description}
                          </option>
                        </optgroup>
                      </select>
                    </div>
                      ...
                      <label className="flex items-center gap-3 cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            setExportOptions({
                              ...exportOptions,
                              applyFXToStems: e.target.checked,
                            })
                          }
                          className="w-4 h-4 accent-indigo-500"
                        />
                        <span className="text-sm text-gray-200">
                          Apply effects to stem exports
                        </span>
                      </label>
                      <p className="text-xs text-gray-500 mt-1 ml-7">
                        When disabled, stems export dry (no EQ/reverb/delay)
                      </p>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <Button
                        ...
                        className="flex-1"
                        disabled={
                          !exportOptions.includeInstrumental &&
                          !exportOptions.includeVocal &&
                          !exportOptions.includeHarmony
                        }
                      >
                        Export Mix
                      </Button>
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-400 mb-4">
                      Save your current mixer settings as a preset for quick
                      recall.
                    </p>
              ...
              {showPresetManager && (
                ...
                >
                  <div
                    className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-lg w-full shadow-2xl max-h-[80vh] overflow-hidden flex flex-col"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      {customPresets.length === 0 ? (
                        <p className="text-sm text-gray-500 italic p-4 text-center bg-gray-800/30 rounded-lg">
                          No custom presets yet. Save your mixer settings or
                          import presets.
                        </p>
                      ) : (
              ...
              {showInstrumentalSwapModal && (
                <div
                  className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4"
                  onClick={() => setShowInstrumentalSwapModal(false)}
                >
                  <div
                    className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-md w-full shadow-2xl"
                    onClick={(e) => e.stopPropagation()}
                  >
                    ...
                    <p className="text-sm text-gray-400 mb-4">
                      Your vocals, refined vocals, and harmony tracks will be
                      preserved.
                    </p>
            ...
            <Card>
              <div className="text-center py-8">
                <p className="text-gray-400">
                  Generate a vocal track below to unlock the Studio Mixer.
                </p>
              </div>
            </Card>
          )}
          ...
          <Card>
            ...
            <p className="text-gray-400 mb-4">
              Select a voice and generate a base vocal performance.
            </p>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  Correction Strength ({pitchCorrectionLevel}%)
                </label>
                <input
                  type="range"
                  min="0"
                  ...
                  onChange={(e) =>
                    setPitchCorrectionLevel(parseInt(e.target.value))
                  }
            ...
            {refiningError && (
              <p className="text-red-400 mt-2 text-sm">{refiningError}</p>
            )}
            {refinedVocalUrl && !isRefining && (
              <div className="mt-4 border-t border-gray-700 pt-4">
                <h4 className="font-semibold mb-2 text-sm text-indigo-300">
                  Refined Vocal Track:
                </h4>
                <WaveformPlayer audioUrl={refinedVocalUrl} />
                <p className="text-xs text-gray-500 mt-1 italic">
                  Enabled in mixer by default.
                </p>
              </div>
            )}
          </Card>
          ...
          <Card>
            ...
            <p className="text-gray-400 text-sm mb-4">
              Create a harmony layer based on your lead vocal track.
            </p>
            ...
            {harmonyError && (
              <p className="text-red-400 mt-2 text-sm">{harmonyError}</p>
            )}
            {harmonyAudioUrl && !isHarmonizing && (
              <div className="mt-4 border-t border-gray-700 pt-4">
                <h4 className="font-semibold mb-2 text-sm text-purple-300">
                  Generated Harmony:
                </h4>
