...
import { analyzeRhymeAndMeter, generateLineAlternatives } from '../../services/geminiService';
...
const LyricLab: React.FC<LyricLabProps> = ({ initialLyrics, onUpdateLyrics }) => {
    ...
    const handleGenerateAlternatives = async () => {
        ...
        try {
            const alts = await generateLineAlternatives(lines[selectedLineIndex], rewriteGoal);
    ...
    return (
        <Page title="Lyric Lab" description="Fine-tune your lyrics with AI-powered rhyme analysis and magic rewrites.">
                ...
                <Card className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto bg-gray-900/50 rounded-lg p-4 font-mono text-lg leading-relaxed">
                        {lines.map((line, i) => (
                            ...
                            >
                                {line || <span className="text-gray-600 italic text-sm">-- Empty Line --</span>}
                            </div>
                        ))}
                    </div>
                     <textarea 
                ...
                <div className="flex flex-col gap-6">
                     <Card>
                        ...
                        {selectedLineIndex !== null ? (
                            isAnalyzing ? (
                                <p className="text-gray-400 animate-pulse">Analyzing meter and rhyme...</p>
                            ) : (
                            ...
                            <p className="text-gray-500 italic">Select a line to see analysis.</p>
                        )}
                    </Card>
                    ...
                    <Card className="flex-1 flex flex-col">
                        <h3 className="text-xl font-semibold mb-4">Magic Rewrite</h3>
                         <div className="flex gap-2 mb-4">
                            ...
                            >
                                ...
                                <option value="Make it More Abstract">Make it More Abstract</option>
                                <option value="Make it Punchier">Make it Punchier</option>
                            </select>
                            <Button onClick={handleGenerateAlternatives} disabled={selectedLineIndex === null || isGeneratingAlts} isLoading={isGeneratingAlts}>
                                Rewrite
                            </Button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2">
                             {alternatives.map((alt, i) => (
                                 <div key={i} className="flex justify-between items-center p-3 bg-gray-700/50 rounded-lg group">
                                     <p className="text-sm text-gray-200">{alt}</p>
                                     <Button size="sm" variant="ghost" onClick={() => handleApplyAlternative(alt)} className="opacity-0 group-hover:opacity-100">Apply</Button>
                                 </div>
                             ))}
                             {alternatives.length === 0 && !isGeneratingAlts && (
                                 <p className="text-gray-500 italic text-center mt-8">Select a line and goal to generate options.</p>
                             )}
                        </div>
                    </Card>
                </div>
