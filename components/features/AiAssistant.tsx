...
const LIVE_MODELS = [
  {
    ...
    description: "Optimized for real-time, low-latency voice.",
  },
];
...
const ChatBot: React.FC = () => {
    ...
    try {
      ...
      if (parsedSessions.length > 0) {
        ...
        if (
          ...
          setActiveSessionId(
            parsedSessions.sort((a, b) => b.createdAt - a.createdAt)[0].id,
          );
        }
      } else {
        handleNewChat();
  ...
  const handleDeleteSession = (sessionIdToDelete: string) => {
    ...
    if (activeSessionId === sessionIdToDelete) {
      if (remainingSessions.length > 0) {
        setActiveSessionId(
          remainingSessions.sort((a, b) => b.createdAt - a.createdAt)[0].id,
        );
      } else {
        handleNewChat();
      }
  ...
  const handleSend = async () => {
    if (!input.trim() || isLoading || !activeSessionId || !activeSession)
      return;
    ...
    try {
      ...
      if (targetLanguage && finalModelText.trim()) {
        const translatedModelText = await translateText(
          finalModelText,
          targetLanguage,
        );
        setSessions((prev) =>
          prev.map((s) => {
            if (s.id === activeSessionId) {
              ...
              return {
                ...
                messages: [
                  ...currentMsgs.slice(0, -1),
                  { ...lastMsg, translatedText: translatedModelText },
                ],
              };
            }
            return s;
        ...
        prev.map((s) => {
          if (s.id === activeSessionId) {
            ...
            return {
              ...
              messages: [
                ...currentMsgs.slice(0, -1),
                { role: "model", text: "Sorry, I had trouble responding." },
              ],
            };
          }
          return s;
          ...
          {filteredSessions.map((session) => (
            ...
            >
              <div className="flex-1 truncate">
                <p
                  className={`text-sm font-semibold truncate ${session.id === activeSessionId ? "text-indigo-300" : "text-gray-300"}`}
                >
                  {session.name}
                </p>
          ...
          <>
            <div className="p-4 border-b border-gray-700/50">
              <input
                ...
                onChange={(e) =>
                  updateSession(activeSession.id, { name: e.target.value })
                }
            ...
            {isEditingInstruction ? (
              <div className="flex-1 flex flex-col p-4">
                ...
                <p className="text-sm text-gray-400 mb-2">
                  Define the AI's persona, role, and rules. This instruction is
                  saved per-chat.
                </p>
                <textarea
                      ...
                      >
                        {msg.translatedText ? (
                          <>
                            ...
                            <p className="text-xs text-gray-300/70 mt-1 italic whitespace-pre-wrap">
                              {msg.text}
                            </p>
                          </>
                        ) : (
...
const LiveConversation: React.FC = () => {
  ...
  const [currentModelTranscription, setCurrentModelTranscription] =
    useState("");
  const [targetLanguage, setTargetLanguage] = useState("");
  ...
  useEffect(() => {
    try {
      ...
      const parsedSessions: LiveSession[] = savedSessions
        ? JSON.parse(savedSessions)
        : [];
  ...
  const startSession = async () => {
    if (sessionStatus !== "idle" || !process.env.API_KEY || !activeSession)
      return;
    ...
    const sessionPromise = ai.live.connect({
      ...
      config: {
        ...
        systemInstruction:
          "You are a friendly and helpful creative assistant for a musician.",
    ...
    if (targetLanguage) {
      for (let i = 0; i < historyToSave.length; i++) {
        if (!historyToSave[i].translatedText) {
          historyToSave[i].translatedText = await translateText(
            historyToSave[i].text,
            targetLanguage,
          );
        }
      }
          ...
          {sessions.map((session) => (
            ...
            >
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  sessionStatus === "idle" && handleDeleteSession(session.id);
                }}
          ...
          <>
            <div className="p-4 border-b border-gray-700/50">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mt-2">
                <div className="flex items-center gap-4">
                  <button
                    onClick={
                      sessionStatus === "idle" ? startSession : stopSession
                    }
                  ...
                  <div>
                    ...
                    <Button
                      ...
                      variant="ghost"
                      disabled={sessionStatus !== "idle" || !hasUnsavedChanges}
                    >
                      Save Session
                      {hasUnsavedChanges && (
            ...
            <div className="flex-1 overflow-y-auto bg-gray-900/50 p-4 space-y-4">
              {liveHistory.map((turn) => (
                <div
                  key={turn.id}
                  className={`group flex items-start gap-3 ${turn.speaker === "user" ? "justify-end" : "justify-start"}`}
                >
                  {turn.speaker === "model" && (
                    <ModelIcon className="h-6 w-6 text-indigo-400 flex-shrink-0 mt-1" />
                  )}
                  <div
                    className={`max-w-xs md:max-w-md px-4 py-2 rounded-lg ${turn.speaker === "user" ? "bg-indigo-600 text-white" : "bg-gray-700"}`}
                  >
                    {turn.translatedText ? (
                      <>
                        <p className="text-sm">{turn.translatedText}</p>
                        <p className="text-xs text-gray-300/70 mt-1 italic">
                          {turn.text}
                        </p>
                      </>
                    ) : (
...
const AiAssistant: React.FC = () => {
  ...
  return (
    <Page
      title="AI Assistant"
      description="Get instant help, brainstorm ideas, or just have a chat with your creative partner."
    >
      <Tabs tabs={tabs} />
    </Page>
  );
