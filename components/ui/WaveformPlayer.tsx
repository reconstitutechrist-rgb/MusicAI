...
const ZoomResetIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>;
...
const WaveformPlayer: React.FC<WaveformPlayerProps> = ({ audioUrl }) => {
  ...
  const drawWaveform = useCallback((buffer: AudioBuffer, targetCanvas?: HTMLCanvasElement, isFullView = false) => {
    ...
    const { start, end } = isFullView ? { start: 0, end: 1 } : getVisibleRange();
    ...
    ctx.fillStyle = isFullView ? 'rgba(99, 102, 241, 0.3)' : 'rgba(129, 140, 248, 0.2)';
  ...
  const drawProgress = useCallback(() => {
    ...
    const { duration: audioDuration, currentTime: audioCurrentTime } = audioRef.current;
    const width = canvas.offsetWidth;
    const height = canvas.offsetHeight;
  ...
  const handleMinimapClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (zoom > 1) {
      ...
      const newScroll = Math.max(0, Math.min(maxScroll, (x / width) - visibleWidth / 2));
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const newScroll = Math.max(0, Math.min(maxScroll, dragStartScroll + scrollDelta));
  ...
  return (
    <div className="w-full mt-2 bg-gray-700/50 rounded-lg p-3" ref={containerRef}>
        ...
        <div className="flex items-center justify-between text-gray-300">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-mono w-10 text-center">{zoom.toFixed(1)}x</span>
              <button
