...
export const DEFAULT_EQ_BANDS: EQBand[] = [
  { id: 'band-1', frequency: 80, gain: 0, q: 0.7, type: 'lowshelf', enabled: true },
  { id: 'band-2', frequency: 250, gain: 0, q: 1.4, type: 'peaking', enabled: true },
  { id: 'band-3', frequency: 1000, gain: 0, q: 1.4, type: 'peaking', enabled: true },
  { id: 'band-4', frequency: 4000, gain: 0, q: 1.4, type: 'peaking', enabled: true },
  { id: 'band-5', frequency: 12000, gain: 0, q: 0.7, type: 'highshelf', enabled: true },
];
...
const ParametricEQ: React.FC<ParametricEQProps> = ({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const newBands = bands.map(band =>
      band.id === draggingBand
        ? { ...band, frequency: Math.round(newFreq), gain: Math.round(newGain * 10) / 10 }
        : band
    );
    onChange(newBands);
  };
  ...
  const selectedBandData = selectedBand ? bands.find(b => b.id === selectedBand) : null;
      ...
      {selectedBandData && (
        <div className="mt-3 p-3 bg-gray-800/50 rounded-lg space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => updateSelectedBand({ enabled: !selectedBandData.enabled })}
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="text-xs text-gray-500 block mb-1">Freq (Hz)</label>
              <input
                type="number"
                min={20}
                max={20000}
                value={selectedBandData.frequency}
                onChange={(e) => updateSelectedBand({ frequency: parseInt(e.target.value) || 20 })}
                className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
              />
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-1">Gain (dB)</label>
              <input
                type="number"
                min={-12}
                max={12}
                ...
                onChange={(e) => updateSelectedBand({ gain: parseFloat(e.target.value) || 0 })}
                className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => updateSelectedBand({ q: parseFloat(e.target.value) || 1 })}
                className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => updateSelectedBand({ type: e.target.value as BiquadFilterType })}
              className="w-full bg-gray-900 border border-gray-600 rounded px-2 py-1 text-sm text-gray-200"
            >
