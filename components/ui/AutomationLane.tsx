...
export default function AutomationLane({
  ...
  const [showCurveMenu, setShowCurveMenu] = useState<{
    x: number;
    y: number;
    pointId: string;
  } | null>(null);
  ...
  const getCanvasCoords = useCallback(
    (e: React.MouseEvent): { time: number; value: number } => {
  ...
  const drawLane = useCallback(() => {
    ...
    if (lane.points.length > 0) {
      ...
      sortedPoints.forEach((point) => {
        ...
        ctx.fillStyle =
          selectedPointId === point.id ? "#fff" : CURVE_COLORS[point.curve];
        ctx.fill();
  ...
  const findPointAt = useCallback(
    (time: number, value: number): AutomationPoint | null => {
      const canvas = canvasRef.current;
  ...
  const displayValue = denormalizeValue(
    currentValue,
    lane.minValue,
    lane.maxValue,
  );
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-1 right-2 text-[10px] text-gray-500">
        Shift+click: add | Double-click: add/delete | Right-click: curve | Drag:
        move
      </div>
      ...
      {showCurveMenu && (
        ...
        >
          ...
          {(
            ["linear", "smooth", "exponential", "hold"] as AutomationCurveType[]
          ).map((curve) => (
            <button
              key={curve}
