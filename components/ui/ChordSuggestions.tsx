...
const CHORD_QUALITIES = ['', 'm', '7', 'maj7', 'm7', 'dim', 'aug', 'sus4', 'sus2', 'add9'];
...
const getRelatedKeys = (key: string): { key: string; relationship: string }[] => {
  ...
  const useFlats = key.includes('b') || ['F', 'Bb', 'Eb', 'Ab', 'Db'].includes(root);
  ...
  if (isMinor) {
    ...
    related.push({ key: noteArray[(rootIndex + 3) % 12] + ' Major', relationship: 'Relative Major' });
    // Parallel major
    related.push({ key: root + ' Major', relationship: 'Parallel Major' });
    // Dominant (5 semitones up)
    related.push({ key: noteArray[(rootIndex + 7) % 12] + ' minor', relationship: 'Dominant' });
    // Subdominant (5 semitones down)
    related.push({ key: noteArray[(rootIndex + 5) % 12] + ' minor', relationship: 'Subdominant' });
  } else {
    // Relative minor (3 semitones down)
    related.push({ key: noteArray[(rootIndex + 9) % 12] + ' minor', relationship: 'Relative Minor' });
    // Parallel minor
    related.push({ key: root + ' minor', relationship: 'Parallel Minor' });
    // Dominant key (5 semitones up)
    related.push({ key: noteArray[(rootIndex + 7) % 12] + ' Major', relationship: 'Dominant Key' });
    // Subdominant key (5 semitones down)
    related.push({ key: noteArray[(rootIndex + 5) % 12] + ' Major', relationship: 'Subdominant Key' });
  }
...
const romanToChord = (roman: string, key: string): string => {
  ...
  if (roman.startsWith('b')) {
    ...
    if (degreeIndex !== undefined && degreeIndex >= 0) {
      // Flatten the note
      const useFlats = key.includes('b') || ['F', 'Bb', 'Eb', 'Ab', 'Db'].includes(scaleNotes[0]);
      const noteArray = useFlats ? FLAT_NOTES : NOTES;
...
const ChordSuggestions: React.FC<ChordSuggestionsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'progressions' | 'alternatives' | 'keys'>('progressions');
  ...
  const progressionSuggestions = useMemo(() => {
    ...
    for (const [genre, progs] of Object.entries(GENRE_PROGRESSIONS)) {
      if (genreLower.includes(genre) || genre.includes(genreLower.split(' ')[0])) {
        return progs;
      }
    }
  ...
  const alternativeChords = useMemo(() => {
    ...
    return audioAnalysis.chords.map(chord => {
      ...
      CHORD_QUALITIES.filter(q => q !== quality).slice(0, 4).forEach(q => {
    ...
    return (
      <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
        <div className="text-center text-gray-400">
          ...
          <p className="text-sm mt-1">Analyze audio to get harmonic suggestions</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gray-800/50 rounded-lg border border-gray-700 overflow-hidden">
      ...
      <div className="p-4 border-b border-gray-700 bg-gradient-to-r from-blue-900/30 to-cyan-900/30">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-white">Chord & Harmony Suggestions</h3>
            <p className="text-sm text-gray-400">
              Key: <span className="text-blue-300 font-medium">{audioAnalysis.key}</span> |
              Chords: <span className="text-cyan-300">{audioAnalysis.chords.join(' - ')}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4">
        {activeTab === 'progressions' && (
          <div className="space-y-3">
            <p className="text-xs text-gray-500">Suggested progressions for {audioAnalysis.genre}:</p>
            {progressionSuggestions.map((prog, i) => (
              <div
                key={i}
                onClick={() => setSelectedProgression(selectedProgression === i ? null : i)}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  {selectedProgression === i && (
                    <span className="text-xs text-blue-400">Click chord to copy</span>
                  )}
                </div>
                {selectedProgression === i && (
                  <div className="flex gap-2 mt-2 pt-2 border-t border-gray-600">
                    {prog.map((numeral, j) => (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          const chord = romanToChord(numeral, audioAnalysis.key);
                          onChordSelect?.(chord);
                        }}
        ...
        {activeTab === 'alternatives' && (
          <div className="space-y-3">
            <p className="text-xs text-gray-500">Alternative chords for your progression:</p>
            {alternativeChords.map((item, i) => (
              <div key={i} className="p-3 bg-gray-700/50 rounded-lg">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-lg font-bold text-cyan-400">{item.original}</span>
        ...
        {activeTab === 'keys' && (
          <div className="space-y-3">
            <p className="text-xs text-gray-500">Modulation suggestions from {audioAnalysis.key}:</p>
            {relatedKeys.map((item, i) => (
              ...
              >
                <div>
                  <span className="text-lg font-bold text-white">{item.key}</span>
                  <span className="ml-3 text-sm text-gray-400">{item.relationship}</span>
                </div>
                <button
