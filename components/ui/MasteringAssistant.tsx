...
const GENRE_PROFILES: Record<string, Partial<MasteringSuggestions>> = {
  'hip-hop': {
    ...
    compression: { threshold: -12, ratio: 4, attack: 0.01, release: 0.15, makeupGain: 3 },
  ...
  'pop': {
    ...
    compression: { threshold: -14, ratio: 3, attack: 0.005, release: 0.1, makeupGain: 2 },
  ...
  'rock': {
    ...
    compression: { threshold: -10, ratio: 4, attack: 0.003, release: 0.08, makeupGain: 4 },
    multibandPreset: 'punch',
    targetLufs: -12,
    truePeakLimit: -1,
    stereoWidth: 1.0
  ...
  'electronic': {
    ...
    compression: { threshold: -8, ratio: 5, attack: 0.001, release: 0.05, makeupGain: 5 },
  ...
  'jazz': {
    ...
    compression: { threshold: -18, ratio: 2, attack: 0.02, release: 0.2, makeupGain: 1 },
    multibandPreset: 'gentle',
    targetLufs: -16,
    truePeakLimit: -2,
    stereoWidth: 1.0
  ...
  'classical': {
    ...
    compression: { threshold: -24, ratio: 1.5, attack: 0.05, release: 0.3, makeupGain: 0 },
    multibandPreset: 'gentle',
    targetLufs: -18,
    truePeakLimit: -2,
    stereoWidth: 1.0
  ...
  'r&b': {
    ...
    compression: { threshold: -14, ratio: 3, attack: 0.008, release: 0.12, makeupGain: 2 },
  ...
  'default': {
    ...
    compression: { threshold: -14, ratio: 3, attack: 0.01, release: 0.1, makeupGain: 2 },
    multibandPreset: 'gentle',
    targetLufs: -14,
    truePeakLimit: -1,
    stereoWidth: 1.0
...
const MOOD_ADJUSTMENTS: Record<string, { eq: Partial<MasteringSuggestions['eq']>; stereoWidth: number }> = {
  'energetic': { eq: { highShelf: { freq: 10000, gain: 2 } }, stereoWidth: 1.2 },
  'chill': { eq: { highMid: { freq: 3000, gain: -0.5, q: 2 } }, stereoWidth: 1.1 },
  'dark': { eq: { lowShelf: { freq: 60, gain: 2 }, highShelf: { freq: 8000, gain: -1 } }, stereoWidth: 0.9 },
  'bright': { eq: { highShelf: { freq: 12000, gain: 2.5 } }, stereoWidth: 1.15 },
  'warm': { eq: { lowMid: { freq: 400, gain: 1, q: 1.5 }, highShelf: { freq: 10000, gain: -0.5 } }, stereoWidth: 1.0 },
  'aggressive': { eq: { highMid: { freq: 4000, gain: 2, q: 1.5 } }, stereoWidth: 1.0 },
...
const MasteringAssistant: React.FC<MasteringAssistantProps> = ({
  ...
  const [suggestions, setSuggestions] = useState<MasteringSuggestions | null>(null);
  ...
  const generateSuggestions = useCallback(() => {
    ...
    let moodAdjustment = MOOD_ADJUSTMENTS['default'] || { eq: {}, stereoWidth: 1.0 };
    ...
    reasoning.push(`Detected genre: ${audioAnalysis.genre} - applying ${genreLower.includes('hip') ? 'punchy bass and crisp highs' : 'balanced'} profile`);
    reasoning.push(`Key: ${audioAnalysis.key} - EQ curves optimized for harmonic content`);
    reasoning.push(`BPM: ${audioAnalysis.bpm} - compression timing adjusted for tempo`);
    reasoning.push(`Mood: ${audioAnalysis.mood} - ${moodLower.includes('energetic') ? 'boosting presence and width' : 'maintaining natural dynamics'}`);

    if (audioAnalysis.productionFeedback) {
      reasoning.push(`Production note: ${audioAnalysis.productionFeedback.slice(0, 100)}...`);
    }
    ...
    return (
      <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
        <div className="flex items-center gap-3">
          ...
          <span className="text-gray-300">Analyzing audio for mastering suggestions...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
        <div className="text-center text-gray-400">
          ...
          <p className="text-sm mt-1">Analyze your audio to get AI-powered mastering suggestions</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Genre: {audioAnalysis.genre} | Key: {audioAnalysis.key}</p>
            </div>
          </div>
          {!suggestions && (
            <button
      ...
      {suggestions && (
        <>
          ...
          <div className="p-4 border-b border-gray-700">
            <h4 className="text-sm font-bold text-purple-300 uppercase tracking-wider mb-2">AI Analysis</h4>
          ...
          <div className="p-4 grid grid-cols-2 md:grid-cols-4 gap-4 border-b border-gray-700">
            <div className="text-center p-3 bg-gray-900/50 rounded-lg">
              ...
              <p className="text-xl font-bold text-green-400">{suggestions.targetLufs}</p>
            </div>
            <div className="text-center p-3 bg-gray-900/50 rounded-lg">
              <p className="text-xs text-gray-500 uppercase">True Peak</p>
              <p className="text-xl font-bold text-yellow-400">{suggestions.truePeakLimit} dB</p>
            </div>
            <div className="text-center p-3 bg-gray-900/50 rounded-lg">
              <p className="text-xs text-gray-500 uppercase">Comp Ratio</p>
              <p className="text-xl font-bold text-blue-400">{suggestions.compression.ratio}:1</p>
            </div>
            <div className="text-center p-3 bg-gray-900/50 rounded-lg">
              <p className="text-xs text-gray-500 uppercase">Stereo Width</p>
              <p className="text-xl font-bold text-indigo-400">{(suggestions.stereoWidth * 100).toFixed(0)}%</p>
            </div>
          </div>
          ...
          <div className="border-b border-gray-700">
            ...
            >
              <span className="text-sm font-medium text-gray-300">Detailed Settings</span>
              <svg
            ...
            {showDetails && (
              <div className="p-4 space-y-4 bg-gray-900/30">
                {/* EQ Settings */}
                <div>
                  <h5 className="text-xs font-bold text-gray-400 uppercase mb-2">EQ Recommendations</h5>
                  <div className="grid grid-cols-5 gap-2 text-xs">
                    {(Object.entries(suggestions.eq) as [string, { freq: number; gain: number; q?: number }][]).map(([band, settings]) => (
                      <div key={band} className="bg-gray-800 p-2 rounded text-center">
                        <p className="text-gray-500 capitalize">{band.replace(/([A-Z])/g, ' $1').trim()}</p>
                        <p className="text-white font-mono">{settings.freq}Hz</p>
                        <p className={`font-bold ${settings.gain >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                ...
                <div>
                  <h5 className="text-xs font-bold text-gray-400 uppercase mb-2">Compression Settings</h5>
                  <div className="grid grid-cols-5 gap-2 text-xs">
                    <div className="bg-gray-800 p-2 rounded text-center">
                      <p className="text-gray-500">Threshold</p>
                      <p className="text-white font-bold">{suggestions.compression.threshold}dB</p>
                    </div>
                    <div className="bg-gray-800 p-2 rounded text-center">
                      <p className="text-gray-500">Ratio</p>
                      <p className="text-white font-bold">{suggestions.compression.ratio}:1</p>
                    </div>
                    <div className="bg-gray-800 p-2 rounded text-center">
                      <p className="text-gray-500">Attack</p>
                      <p className="text-white font-bold">{suggestions.compression.attack * 1000}ms</p>
                    </div>
                    <div className="bg-gray-800 p-2 rounded text-center">
                      <p className="text-gray-500">Release</p>
                      <p className="text-white font-bold">{suggestions.compression.release * 1000}ms</p>
                    </div>
                    <div className="bg-gray-800 p-2 rounded text-center">
                      <p className="text-gray-500">Makeup</p>
                      <p className="text-white font-bold">+{suggestions.compression.makeupGain}dB</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Multiband Preset */}
                <div>
                  <h5 className="text-xs font-bold text-gray-400 uppercase mb-2">Recommended Multiband Preset</h5>
