...
export const LUFS_PRESETS: Record<LufsPreset, LufsPresetConfig> = {
  spotify: { name: 'Spotify', target: -14, truePeakLimit: -1, toleranceRange: 2, color: '#1DB954' },
  youtube: { name: 'YouTube', target: -13, truePeakLimit: -1, toleranceRange: 2, color: '#FF0000' },
  apple: { name: 'Apple Music', target: -16, truePeakLimit: -1, toleranceRange: 2, color: '#FC3C44' },
  broadcast: { name: 'Broadcast', target: -24, truePeakLimit: -2, toleranceRange: 1, color: '#3B82F6' },
  custom: { name: 'Custom', target: -14, truePeakLimit: -1, toleranceRange: 2, color: '#8B5CF6' }
};
...
const LufsMeter: React.FC<LufsMeterProps> = ({
  ...
  const calculateIntegratedWithGating = useCallback((blocks: number[]): number => {
    ...
    const ungatedSum = afterAbsoluteGate.reduce((sum, lufs) =>
      sum + Math.pow(10, lufs / 10), 0);
    ...
    const afterRelativeGate = afterAbsoluteGate.filter(lufs => lufs > relativeThreshold);

    if (afterRelativeGate.length === 0) return -Infinity;

    const gatedSum = afterRelativeGate.reduce((sum, lufs) =>
      sum + Math.pow(10, lufs / 10), 0);
  ...
  const calculateLRA = useCallback((blocks: number[]): number | undefined => {
    ...
    const avgSum = afterAbsoluteGate.reduce((sum, lufs) =>
      sum + Math.pow(10, lufs / 10), 0);
    ...
    const lowIdx = Math.floor(sorted.length * 0.10);
  ...
  const draw = useCallback(() => {
    ...
    if (analyser && audioContext) {
      ...
      if (gatedBlocksRef.current.length > 36000) { // 10 min at 60fps
        gatedBlocksRef.current.shift();
      }

      // Store blocks for LRA (every 3 seconds for short-term blocks)
      if (shortTermBufferRef.current.length === 180) {
        const shortTermAvg = shortTermBufferRef.current.reduce((a, b) =>
          a + Math.pow(10, b / 10), 0) / shortTermBufferRef.current.length;
        const shortTermBlock = shortTermAvg > 0 ? 10 * Math.log10(shortTermAvg) : -Infinity;
        lraBlocksRef.current.push(shortTermBlock);
        if (lraBlocksRef.current.length > 200) { // Keep ~10 minutes
      ...
      const integratedLufs = calculateIntegratedWithGating(gatedBlocksRef.current);
    ...
    ctx.fillText(readings.truePeak > -100 ? readings.truePeak.toFixed(1) + ' dBTP' : '-∞', panelX + 5, meterY + 117);
    ...
    ctx.font = 'bold 12px monospace';
    ctx.fillText(readings.range !== undefined ? readings.range.toFixed(1) + ' LU' : '—', panelX + 5, meterY + 144);
    ...
    animationFrameRef.current = requestAnimationFrame(draw);
  }, [analyser, audioContext, width, height, targetPreset, readings, calculateMeanSquare, meanSquareToLufs, calculateIntegratedWithGating, calculateLRA]);
